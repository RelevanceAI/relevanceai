...
notebook = Path.cwd() / 'examples' / 'RelevanceAI_ReadMe_Quickstart_Text_to_Image_Search.ipynb'
...
CLIENT_INSTANTIATION_STR_REPLACE = f'(project=\\"{TEST_PROJECT}\\", api_key=\\"{TEST_API_KEY}\\")'
...
for notebook in Path(DOCS_PATH).glob('**/*.ipynb'):
    ...
    notebook_find_replace(notebook, PIP_INSTALL_SENT_REGEX, PIP_INSTALL_STR_REGEX, PIP_INSTALL_STR_REPLACE)

    ## Temporarily updating notebook with test creds
    notebook_find_replace(notebook, CLIENT_INSTANTIATION_SENT_REGEX, CLIENT_INSTANTIATION_STR_REGEX, CLIENT_INSTANTIATION_STR_REPLACE)
    ...
    notebook_find_replace(notebook, CLIENT_INSTANTIATION_SENT_REGEX, CLIENT_INSTANTIATION_STR_REGEX, CLIENT_INSTANTIATION_BASE)
