...
def preprocess_linear_doc(doc):
    ...
    if doc["completedAt"]:
        completed_at = datetime.datetime.strptime(
            doc["completedAt"], "%Y-%m-%dT%H:%M:%S.%fZ"
        )
        doc["completed"] = 1
        if doc["startedAt"]:
            doc["timeBetweenStartToComplete"] = (
    ...
    if doc["canceledAt"]:
        completed_at = datetime.datetime.strptime(
            doc["canceledAt"], "%Y-%m-%dT%H:%M:%S.%fZ"
        )
        doc["canceled"] = 1
...
def linear_recipe(dataset_id, api_key, client):
    ...
    results = requests.post(
        ...
        json={"query": query_string.format(after="")},
    ).json()

    docs += [
        preprocess_linear_doc(n["node"]) for n in results["data"]["issues"]["edges"]
    ]
        ...
        results = requests.post(
            ...
            json={
                "query": query_string.format(
                    after=f', after: "{results["data"]["issues"]["pageInfo"]["endCursor"]}"'
                )
            },
        ).json()
        docs += [
