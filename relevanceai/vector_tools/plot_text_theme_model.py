...
class PlotTextThemeModel(BatchAPIClient, BaseTextProcessing, LoguruLogger, DocUtils):
    ...
    def build_and_plot_clusters(
        ...
        dr_centers = self.dim_reduction(vector_data = vectors, embedding_dims = self.embedding_dims, k = self.dim_red_k, 
            n_epochs_without_progress = self.n_epochs_without_progress, run_fit = False)
        ...
        self.plot_clusters(docs, dr_docs, centers, cluster_data, vector_field, alias, cluster_representative_cnt, 
            plot_axis = plot_axis, figsize = figsize, cmap = cmap, alpha = alpha)
    ...
    def get_documents(self, vector_field:str, text_field:str, max_doc_num: int = None):
        ...
        if max_doc_num:
            ...
            docs = self.get_all_documents(dataset_id = self.dataset_id, filters = filters, select_fields = fields)
        return docs

    def batch_load_docs(self, fields:List[str], filters:List[dict] = [], page_size:int = 200, cursor:str = None):
        return self.datasets.documents.get_where(dataset_id=self.dataset_id, select_fields=fields, 
    ...
    def kmeans_clustering(self, docs, vector_field = str, k : int = 10, alias: str = "kmeans"):
        ...
        res = self.update_documents(self.dataset_id, clustered_docs, chunksize = self.upload_chunksize)
        centers = clusterer.get_centroid_docs()
        self.services.cluster.centroids.insert(dataset_id = self.dataset_id, cluster_centers = centers,
    ...
    def get_cluster_word_freq(self, cluster_data: dict, remove_stop_words: bool = True, 
        ...
        for cluster in tqdm(cluster_data):
            cluster_data[cluster]['word_freq'] = self.get_word_frequency(str_list = cluster_data[cluster]['data'], 
                remove_stop_words = remove_stop_words, additional_stop_words = additional_stop_words, language= self.language)
        return {c:[cluster_data[c]['word_freq'][:cluster_representative_cnt]] for c in cluster_data}  

    def dim_reduction(self, vector_data: List, embedding_dims=2, k=800, n_epochs_without_progress=100, run_fit = True):
    ...
    def plot_clusters(self, docs:List[dict], dr_docs:List, centers:List[dict], cluster_data:dict, 
        vector_field:str, alias:str, cluster_representative_cnt:int, plot_axis: str = 'off',
        figsize : Tuple[int, ...] = (20,10), cmap: str = "plasma", alpha : float = 0.2):
        ...
        plt.scatter([x[0] for x in dr_docs], [x[1] for x in dr_docs], c=color, cmap=cmap, alpha=alpha)
