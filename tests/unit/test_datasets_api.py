...
class TestDatset:
    def test_info(self, test_dataset_df):
        info = test_dataset_df.info()
        assert True

    def test_shape(self, test_dataset_df):
        shape = test_dataset_df.shape
        assert True

    def test_head(self, test_dataset_df):
        head = test_dataset_df.head()
        assert True

    def test_describe(self, test_dataset_df):
        description = test_dataset_df.describe()
        assert True

    def test_cluster(self, test_dataset_df):
        centroids = test_dataset_df.cluster(
            field="sample_1_vector_", 
            overwrite=True
        )
        assert True

    def test_groupby_agg(self, test_dataset_df):
        agg = test_dataset_df.agg({"sample_1_label": "avg"})
        groupby_agg = test_dataset_df.groupby(
            ["sample_1_description"]).agg(
                {"sample_1_label": "avg"}
        )
        assert True

    def test_centroids(self, test_clustered_dataset_df):
    ...
    def test_sample(self, test_dataset_df):
    ...
    def test_custom_cluster_with_alias(self, test_client: Client, test_sample_vector_dataset):
        """
        ...
        df.cluster("sample_1_vector_", clusterer=kmeans, 
            overwrite=True, alias=ALIAS)
        db_health = test_client.datasets.monitor.health(test_sample_vector_dataset)
        assert "_cluster_" in db_health
