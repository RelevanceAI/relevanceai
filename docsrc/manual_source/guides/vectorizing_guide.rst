...
vector. This new vector\_field is always called
``f"_dim{n_dims}_feature_vector_"``, with n\_dims being the size of the
concatenated vector.
...
schema with ``_text_`` denoting text\_fields and ``_image_`` denoting
image fields.
